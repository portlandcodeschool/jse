...
function makeWatcher(val) {
    ...
    function watchXFn(cb) {
        watchers.push(cb);
        return x;
    }
    function changeXFn(val) {
        x = val;
        watchers.forEach(function(cb) { //do all the things!
            cb(x);
        });
    }
    return {watchX:watchXFn, changeX:changeXFn};
...
var watcher = makeWatcher(7);
watcher.watchX(function(x) {console.log('Hey, Dan: x is now '+x)});//7
watcher.watchX(function(x) {alert('X has changed to '+x)});//7
var x2 = watcher.watchX(function(x) {x2 = x*x;});
x2 *= x2;
watcher.changeX(9);//undef
x2;
/*
81
*/
...
*/
// has equivalent:
result  = (function(x,y) {return x+y}) (1,2);
/*
3
*/
...
var widget = makeThing(7);
var widget2 = makeThing(8);
